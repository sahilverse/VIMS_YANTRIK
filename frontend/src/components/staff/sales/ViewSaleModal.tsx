      ...
      >
        <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50 sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-extrabold tracking-tight">{sale?.invoiceNumber || 'Loading...'}</h2>
              {sale && (
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest mt-1 ${getStatusColor(sale.paymentStatus)}`}>
                  {sale.paymentStatus}
                </span>
              )}
            </div>
        ...
        {isLoading || !sale ? (
          ...
          <div className="p-6 space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <p className="text-sm font-bold text-zinc-900">{sale.customerName}</p>
            </div>
            ...
            <div className="bg-zinc-50 rounded-2xl p-4 border border-zinc-100">
              ...
              <p className="text-sm font-bold text-zinc-900">
                {new Date(sale.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            </div>
            ...
            <div className="bg-zinc-50 rounded-2xl p-4 border border-zinc-100 col-span-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Processed By</p>
              </div>
              <p className="text-sm font-bold text-zinc-900">{sale.employeeName}</p>
            </div>
          </div>

          <div>
            <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-4">Items Sold</h3>
